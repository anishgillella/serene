...
const MonthSection: React.FC<{ month: string; conflicts: Conflict[]; onStatusChange?: (id: string, status: string) => void }> = ({ month, conflicts, onStatusChange }) => {
        ...
        <div className="relative pb-12 last:pb-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {visibleConflicts.map((conflict) => (
                    <ConflictCard key={conflict.id} conflict={conflict} onStatusChange={onStatusChange} />
                ))}
            </div>
...
const Timeline: React.FC<TimelineProps> = ({ conflicts, onStatusChange }) => {
    ...
    return (
        <div className="relative pl-8 border-l-2 border-border-subtle ml-4 md:ml-8 space-y-0">
            {months.map((month) => (
                <MonthSection key={month} month={month} conflicts={groupedConflicts[month]} onStatusChange={onStatusChange} />
            ))}
        </div>
    );
};
